...
# 사용자 정의 CSS 추가 (전체 배경 + 제목 폰트)
page_bg = """
<style>
[data-testid="stAppViewContainer"] {
    background-color: #004225; /* 딥그린 배경 */
}
...
# 체형별 코디 추천 데이터 + 색감 테마
tips = {
    "사과형 🍎 (상체가 발달한 체형)": ("상체는 심플하게, 하체에 포인트를 주는 것이 좋아요. A라인 스커트, 와이드 팬츠 추천!", "#FF6B6B"),
    "배형 🍐 (하체가 발달한 체형)": ("밝은 톤 상의 + 어두운 톤 하의로 시선을 위로 끌어올리세요. 화려한 액세서리도 효과적!", "#FFD93D"),
    "직사각형 ▭ (허리라인이 잘 안 보이는 체형)": ("허리를 강조하는 벨트, 하이웨이스트 팬츠, 크롭탑 스타일링 추천!", "#6BCB77"),
    "모래시계형 ⏳ (균형잡힌 체형)": ("몸의 라인을 살려주는 원피스, 슬림핏 의상이 잘 어울립니다. 대부분의 스타일 소화 가능!", "#4D96FF"),
    "역삼각형 🔻 (어깨가 넓은 체형)": ("브이넥 상의로 어깨를 좁아보이게, 플레어 스커트나 부츠컷 바지로 하체 볼륨감을 주세요.", "#BC6FF1")
}
...
st.markdown(
    f"""
    <div style='background-color:{color}; padding:15px; border-radius:15px; color:white; font-size:18px; box-shadow: 2px 2px 10px #00000033;'>
        {message}
    </div>
    """,
